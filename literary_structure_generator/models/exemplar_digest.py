...
class Motif(BaseModel):
    ...
    anchors: List[int] = Field(default_factory=list, description="Token positions where motif appears")
...
class ExemplarDigest(BaseModel):
    ...
    schema_version: str = Field(default="ExemplarDigest@2", description="Schema version identifier", alias="schema")
    ...
    event_scripts: List[EventScript] = Field(default_factory=list, description="Common event scripts")
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "discourse": {"beats": [{"id": "cold_open", "span": [0, 180], "function": "tonal hook"}]},
            }
        }
