...
def check_beat_length_adherence(
    ...
    for i, (beat_text, beat_spec) in enumerate(zip(beat_texts, beat_specs)):
...
def estimate_scene_summary_ratio(text: str) -> float:
    ...
    scene_ratio = scene_paras / len(paragraphs)
...
def evaluate_formfit(text: str, spec: StorySpec) -> Dict[str, any]:
    ...
    function_score, function_details = check_beat_function_alignment(
        beat_texts,
        spec.form.beat_map
    )
