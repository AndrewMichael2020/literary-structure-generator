...
def aggregate_scores(
    ...
    overlap_guard = OverlapGuard(
        max_ngram=overlap_result["max_ngram"], overlap_pct=overlap_result["overlap_pct"]
    )
...
def extract_per_beat_scores(results: dict[str, any], spec: StorySpec) -> list[PerBeatScore]:
    ...
    for i, beat_spec in enumerate(spec.form.beat_map):
        ...
        if i < len(per_beat_length):
            ...
            length_note = (
                f"Target: {length_data['target_words']}w, Actual: {length_data['actual_words']}w"
            )
        else:
            length_score = 0.5
        ...
        per_beat_scores.append(
            PerBeatScore(id=beat_spec.id, stylefit=beat_stylefit, formfit=length_score, notes=notes)
        )
...
def create_repro_info(config: GenerationConfig) -> Repro:
    ...
    model = "mock"  # Default
    temp = (
        config.per_beat_generation.temperature[0] if config.per_beat_generation.temperature else 0.0
    )

    return Repro(git_commit=git_commit, model=model, model_temp=temp)
...
def evaluate_draft(
    ...
    return EvalReport(
        ...
        seeds={"global": config.seed, "per_beat": draft.get("seeds", {}).get("per_beat", [])},
        length={
            "words": word_count,
