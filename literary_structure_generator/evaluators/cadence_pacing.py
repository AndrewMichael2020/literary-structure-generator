...
def calculate_cadence_match(actual_dist: dict[str, float], target_dist: dict[str, float]) -> float:
    """
...
def check_paragraph_variance(
    para_lengths: list[int], target_variance: float, tolerance: float = 0.2
) -> float:
    """
...
def estimate_valence_smoothness(text: str) -> float:
    ...
    for para in paragraphs:
        ...
        valence = 0.0 if total == 0 else (pos_count - neg_count) / total
    ...
    return max(0.0, 1.0 - (avg_transition / 2.0))
...
def evaluate_cadence_pacing(text: str, spec: StorySpec) -> dict[str, any]:
    ...
    target_cadence_label = spec.form.beat_map[0].cadence if spec.form.beat_map else "mixed"
    ...
    overall = cadence_score * 0.4 + variance_score * 0.3 + smoothness_score * 0.3
