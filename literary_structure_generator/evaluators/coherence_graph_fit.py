            ...
            if word and word[0].isupper() and len(word) > 1:
                ...
                if clean_word and clean_word not in ['I', 'The', 'A', 'An']:
                    ...
                    if clean_word.lower() in ['hospital', 'clinic', 'office', 'building', 'house']:
                        entity_type = 'PLACE'
                    elif clean_word.endswith('s') or clean_word.lower() in ['company', 'department']:
                        entity_type = 'ORG'
                    else:
        ...
        if mentions >= threshold:
            ...
            if len(positions) > 1:
                ...
                if position_range < len(positions) * 2:
                    issues.append(f"Name spike: {entity} appears {mentions} times in concentrated region")
                    penalty += 0.15
...
def check_entity_introduction(text: str, entity_map: Dict[str, Dict]) -> float:
    ...
    early_count = 0
    for entity, metadata in major_entities:
    ...
    introduction_score = early_count / len(major_entities) if major_entities else 1.0
    
    return introduction_score
